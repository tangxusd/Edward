...
Deno.serve(async req => {
  ...
  try {
    ...
    const requestPayload: Record<string, unknown> = { req_date: reqDate, req_seq_id: reqSeq, huifu_id: merchantId, acct_id: accountId, goods_desc: String(body.goodsDesc || "Edward订阅").slice(0, 127), trade_type: channel.tradeType, trans_amt: amount.toFixed(2), time_expire: timeExpire, delay_acct_flag: "N", term_div_coupon_type: "0", limit_pay_type: "NO_CREDIT", pay_scene: "02", remark: String(body.remark || "Edward支付验证").slice(0, 127), risk_check_data: riskData, terminal_device_data: deviceData, combinedpay_data: combinedPayData, notify_url: notify };
