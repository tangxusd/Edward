...
Deno.serve(async (req) => {
  ...
  const { data, error } = await client.from("resources").select("id,component_id,target,tab_key,category_id,name,summary,detail_markdown,required_plan,status,visibility,created_at,updated_at").eq("id", id).eq("status", "published").maybeSingle();
