...
export function buildExportCommand(request: ExportRequest): string[] {
  ...
  if (request.overlays?.length) {
    const filters = [`scale=${request.width}:${request.height}`, ...request.overlays.map((overlay) => `drawtext=text='${overlay.text.replace(/([\\'])/g, '\\$1')}':fontcolor='${overlay.color ?? '#ffffff'}':fontsize=${overlay.fontSize ?? 48}:enable='between(t,${overlay.start},${overlay.start + overlay.duration})'`)].join(',');
