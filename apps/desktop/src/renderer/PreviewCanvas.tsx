...
export function PreviewCanvas({ project, onChange, onSelect }: { project?: Project; onChange?: (project: Project) => void; onSelect?: (clipId: string) => void }): React.JSX.Element {
  ...
  const updateAt = (clientX: number, clientY: number) => {
    ...
    if (active.mode === 'project-drag' && currentProject) {
      ...
      const peers = projectVisualClips.filter(({ clip }) => clip.id !== active.id).map(({ clip }, index) => clip.layout ?? projectVisualRects[index] ?? active.rect);
  ...
  return <section ref={canvasRef} aria-label="预览画布" onMouseMove={(event) => updateAt(event.clientX, event.clientY)} onMouseUp={finish} style={{ position: 'relative', aspectRatio: '16 / 9', background: '#151923', overflow: 'hidden' }}>{resourceId ? <div aria-label={`预览背景 ${resourceId}`} style={{ position: 'absolute', inset: 0, background: '#243b53' }} /> : null}<i aria-hidden="true" style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, borderLeft: '1px dashed #6f86b0' }} /><i aria-hidden="true" style={{ position: 'absolute', top: '50%', left: 0, right: 0, borderTop: '1px dashed #6f86b0' }} />{guides.x.map((x) => <i key={`x-${x}`} aria-label="垂直对齐参考线" style={{ position: 'absolute', left: x, top: 0, bottom: 0, borderLeft: '1px solid #24b47e', pointerEvents: 'none' }} />)}{guides.y.map((y) => <i key={`y-${y}`} aria-label="水平对齐参考线" style={{ position: 'absolute', top: y, left: 0, right: 0, borderTop: '1px solid #24b47e', pointerEvents: 'none' }} />)}<div role="toolbar" aria-label="卡片布局"><button onClick={() => setLayout(1)} aria-pressed={cardCount === 1}>单卡</button><button onClick={() => setLayout(2)} aria-pressed={cardCount === 2}>双卡</button><button onClick={() => setLayout(3)} aria-pressed={cardCount === 3}>三卡</button></div>{projectVisualClips.map(({ clip, kind }, index) => { const rect = projectVisualRects[index]; const subtitleStyle = kind === '字幕' ? project?.subtitleStyle : undefined; return rect ? <div key={clip.id} aria-label={`项目${kind} ${clip.id}`} onMouseDown={(event) => startProjectDrag(event, clip.id, rect)} onMouseMove={(event) => updateAt(event.clientX, event.clientY)} onClick={() => { persistProjectLayout(clip.id, rect); onSelect?.(clip.id); }} style={{ position: 'absolute', left: rect.x, top: rect.y, width: rect.width, height: rect.height, border: `1px solid ${cardColors[index % cardColors.length]}`, color: subtitleStyle?.color ?? 'white', background: subtitleStyle?.background, fontFamily: subtitleStyle?.fontFamily, fontSize: subtitleStyle?.fontSize, touchAction: 'none', userSelect: 'none' }}>{kind === '卡片' || kind === '字幕' ? clipText(clip.content) : graphicText(clip.content)}<button aria-label={`${kind}缩放控件 ${clip.id}`} onMouseDown={(event) => startProjectResize(event, clip.id, rect)}>缩放</button></div> : null; })}</section>;
}
