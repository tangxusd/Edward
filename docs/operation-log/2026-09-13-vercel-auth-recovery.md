...
- 2026-09-14 预览同步加固：主绘制路径使用同一份 `visibleClipsAt(t)` 快照，并在异步组件同步前按 `clipId` 同步隐藏所有非活动 DOM 宿主，消除最后时间点和空白区间残留旧组件的竞态。
- 2026-09-14 组件动画/挂载修正：增加按片段 ID 的 pending mount 去重，避免播放刷新期间重复创建组件实例；预览恢复 CSS 动画，导出传入 viewport 时按片段局部时间暂停并定位动画帧，避免静止预览或导出动画加速。
- 2026-09-14 交错叠加修正：异步挂载完成后不再因旧同步代际自动销毁仍处于目标活动集合的组件；仅当片段确实离开活动集合时销毁，从而保留交错片段的叠加显示。
- 2026-09-14 同名导出覆盖：Qt 下载接管器在接受导出前删除用户明确指定的同名文件；目标不是普通文件或删除失败时取消下载，不自动改名写入旁边文件。
...
- 2026-09-14 支付订单审计：新增 `payment_audit_events`，逐条记录支付宝创建请求/响应、异步通知和客户端查询的阶段、HTTP 状态、业务码、交易状态和脱敏业务载荷；私钥和请求签名不落库。远程迁移已应用，三个支付宝函数已重新部署。
- 2026-09-14 支付状态对账：`alipay-query-order` 在轮询 pending 订单时主动调用 `alipay.trade.query`，按交易状态和金额核对后更新订单并执行 `apply_paid_order`；客户端收到 `paid` 后清空二维码、关闭弹窗并提示权益已开通。
- 2026-09-14 续费与退款周期：新增 `subscription_periods`，每一笔已支付订单对应一段不可重叠的订阅周期。后续付款从现有 `current_period_end` 开始追加。`alipay-refund-order` 只允许退款 `scheduled`（尚未开始的下一周期）订单，当前/已开始周期在服务端拒绝；支付宝退款成功后才缩短订阅结束时间。迁移与函数已部署。
