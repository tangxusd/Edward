...
- 2026-09-13：修复注册按钮无可见反馈：登录/注册失败或成功均显示 Qt Toast；窗口在 `show()` 前设置 `QWindow::Maximized`，避免普通窗口先显示后最大化的闪烁。`edward_app` 构建通过。
- 2026-09-13：修复注册 400 仅显示底层网络错误的问题：客户端解析 Edge Function 的错误码并显示中文原因；注册输入提示改为“邮箱（登录可使用账号名）”，避免用户误以为注册可直接使用任意账号名。`edward_app` 构建通过。
- 2026-09-13：修复登录错误反馈（解析 `invalid_credentials`，不再只显示 Host requires authentication）；新增密码找回入口；登录与注册拆分为两个对话框，注册使用青色配色；Qt 应用在显示前设置最大化。`edward_app` 构建通过。
- 2026-09-13：修复认证回调与启动窗口：密码找回和注册确认邮件改用本地服务回调（不再跳转 `localhost:3000`），本地服务增加回调提示页；启动改用 `showMaximized()`。注册函数已重新部署，Qt 构建通过。
- 2026-09-13：关闭 Edward 启动时 Resolve 自动连接失败提示（后台仍保留重连）；新增在线 Supabase `auth-recovery` 回调页，密码找回可在未启动本地服务时完成，注册确认也使用该 HTTPS 地址；已清理 Supabase Auth 中全部 4 个用户并核对剩余 0 个。清理为用户明确授权的破坏性操作。
- 2026-09-13：注册流程改为邮箱确认门槛：未确认用户可用同邮箱重试并触发确认邮件重发；已确认邮箱才返回已注册；若 Supabase 未启用邮箱确认则删除临时用户并返回配置错误，不产生有效账号。函数部署、Deno 类型检查和 Qt 构建通过。
