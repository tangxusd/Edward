...
发布级隔离需要一个 Edward 可验证的解释器路径。PATH 中用户任意 Node 无法同时满足可验证来源与最小文件权限。因此发布版应使用 Edward 分发并校验的 Node 运行时，适配器及其 npm 依赖仍由用户独立安装。该运行时体积、许可证和更新策略属于发行包决策，不能在未完成打包合同前悄悄加入。
