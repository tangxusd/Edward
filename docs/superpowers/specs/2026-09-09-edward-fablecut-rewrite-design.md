...
- 系统操作界面支持中文和英文；开发阶段默认使用中文。
- 界面文案、菜单、属性栏、错误提示和导出状态必须进入可切换的本地化资源；用户组件内容、项目名称、素材名称和用户代码保持原文。
...
- 界面语言由 Edward 设置传给 FableCut，FableCut 使用中文/英文两套界面资源；默认语言为中文，用户可在设置中切换并即时生效或重启后生效。
...
6. 增加中文/英文界面资源、语言设置、缺失翻译回退和双语界面回归测试。
...
- 中文和英文界面均覆盖主窗口、时间线、属性栏、导入、导出、错误提示和恢复流程；缺失翻译回退到英文或中文基线，不显示空文案。
