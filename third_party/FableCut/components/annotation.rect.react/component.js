...
function values(props, time, viewport) {
  ...
  const duration = Math.max(0.1, Number(props.duration ?? 2));
  const progress = props.progress == null ? clamp(time / duration, 0, 1) : clamp(Number(props.progress), 0, 1);
  return { width, height, progress, x: Number(props.x ?? 0.5), y: Number(props.y ?? 0.5), boxWidth: width * Number(props.width ?? 0.56), boxHeight: height * Number(props.height ?? 0.28), borderWidth: Number(props.borderWidth ?? 2), radius: Number(props.radius ?? 5), color: props.color || "#1683ff" };
}
