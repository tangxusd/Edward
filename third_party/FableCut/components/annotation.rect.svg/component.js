...
export function mount({ host, props, time, viewport }) {
  ...
  const render = (next, at, size) => {
    ...
    rect.setAttribute("x", "0"); rect.setAttribute("y", "0"); rect.setAttribute("width", String(rw)); rect.setAttribute("height", String(rh)); rect.setAttribute("rx", String(radius)); rect.setAttribute("fill", "none"); rect.setAttribute("stroke", next.color || "#1683ff"); rect.setAttribute("stroke-width", String(Number(next.borderWidth ?? 2))); rect.setAttribute("pathLength", "1"); rect.setAttribute("stroke-dasharray", "1"); rect.setAttribute("stroke-dashoffset", String(1 - p));
  };
